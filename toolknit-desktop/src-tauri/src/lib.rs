...
fn get_install_config() -> Result<InstallConfig, String> {
    ...
    if config_file.is_none() {
        let default_path = dirs::document_dir()
            .map(|d| d.join("HY").to_string_lossy().to_string())
            .unwrap_or_default();
...
async fn download_ffmpeg_runtime(
    ...
    let client = reqwest::Client::builder()
        .user_agent("HY/1.4 ffmpeg-runtime-manager")
...
fn get_whisper_cli_path() -> Result<std::path::PathBuf, String> {
    ...
    Err("Offline transcription engine is unavailable. Please reinstall HY.".to_string())
}
...
async fn download_transcription_model(
    ...
    let client = reqwest::Client::builder()
        .user_agent("HY/1.4 offline-model-manager")
        .build()
...
#[tauri::command]
async fn transform_image_batch(
    app_handle: tauri::AppHandle,
    input_paths: Vec<String>,
    output_dir: String,
    ...
    let _conversion_guard = begin_conversion()?;
    tokio::task::spawn_blocking(move || {
        transform_image_batch_blocking(app_handle, input_paths, output_dir, options)
    })
    .await
    .map_err(|error| format!("Image transform worker failed: {}", error))?
}

fn apply_image_transform(
    img: image::DynamicImage,
    options: &ImageTransformOptions,
) -> Result<Vec<(String, image::DynamicImage)>, String> {
    match options.op.as_str() {
        "resize" => {
            let keep_aspect = options.keep_aspect.unwrap_or(true);
            let width = options.width.unwrap_or(0);
            let height = options.height.unwrap_or(0);
            if width == 0 && height == 0 {
                return Err("Provide a positive width and/or height".to_string());
            }
            if width > 12000 || height > 12000 {
                return Err("Target dimension is too large".to_string());
            }
            let (tw, th) = if keep_aspect {
                let (ow, oh) = (img.width(), img.height());
                if width > 0 && height > 0 {
                    let scale = f64::min(width as f64 / ow as f64, height as f64 / oh as f64);
                    (
                        ((ow as f64) * scale).round().max(1.0) as u32,
                        ((oh as f64) * scale).round().max(1.0) as u32,
                    )
                } else if width > 0 {
                    let scale = width as f64 / ow as f64;
                    (width, ((oh as f64) * scale).round().max(1.0) as u32)
                } else {
                    let scale = height as f64 / oh as f64;
                    (((ow as f64) * scale).round().max(1.0) as u32, height)
                }
            } else {
                (
                    if width > 0 { width } else { img.width() },
                    if height > 0 { height } else { img.height() },
                )
            };
            Ok(vec![(
                "resized".to_string(),
        ...
        "rotate" => {
            let degrees = options.degrees.unwrap_or(0);
            let rotated = match degrees {
                90 => img.rotate90(),
                180 => img.rotate180(),
                270 => img.rotate270(),
                _ => return Err("Rotate degrees must be 90, 180, or 270".to_string()),
            };
            Ok(vec![("rotated".to_string(), rotated)])
        }
        "flip" => {
            let axis = options.axis.as_deref().unwrap_or("");
            let flipped = match axis {
                "horizontal" => img.fliph(),
                "vertical" => img.flipv(),
                _ => return Err("Flip axis must be horizontal or vertical".to_string()),
            };
            Ok(vec![("flipped".to_string(), flipped)])
        }
        "crop" => {
        ...
        "grid_split" => {
            ...
            let cols = options.cols.unwrap_or(0);
            if rows == 0 || cols == 0 || rows > 10 || cols > 10 {
                return Err("Grid rows/cols must be 1-10".to_string());
            }
            let cell_w = img.width() / cols;
            let cell_h = img.height() / rows;
            if cell_w == 0 || cell_h == 0 {
                return Err("Image is too small for the requested grid".to_string());
            }
        ...
        _ => Err(format!("Unsupported transform: {}", options.op)),
    }
}

fn transform_image_batch_blocking(
    app_handle: tauri::AppHandle,
    input_paths: Vec<String>,
    output_dir: String,
    options: ImageTransformOptions,
    ...
    for (i, input_path) in input_paths.iter().enumerate() {
        ...
        match image::open(input) {
            Ok(img) => match apply_image_transform(img, &options) {
                Ok(outputs) => {
                    let mut file_ok = true;
                    for (suffix, out_img) in outputs {
                        if CANCEL_FLAG.load(Ordering::SeqCst) {
                            file_ok = false;
                            break;
                        }
                        let out_stem = if options.op == "grid_split" {
                            format!("{}-{}", stem, suffix)
                        } else {
                            format!("{}-{}", stem, options.op)
                        };
                        let output_path = get_unique_output_path(&output_dir_path, &out_stem, out_ext);
                        let temporary_output_path = output_dir_path.join(format!(
                            ".{}-HY-transform-{}-{}.tmp",
                            out_stem,
                            std::process::id(),
                        ...
                        let save_result = out_img.save_with_format(&temporary_output_path, format);
                        ...
                        if let Err(error) =
                            publish_image_output(&temporary_output_path, &output_path)
                        {
                            file_ok = false;
                            fail_count += 1;
                            errors.push(format!("{}: {}", file_name, error));
...
async fn convert_video_file(
    ...
    command
        ...
        .arg("-pix_fmt")
        .arg("yuv420p");
    let crf_value = quality_crf.unwrap_or(23).clamp(14, 40).to_string();
    match video_encoder.as_str() {
        ...
        "libx264" => {
            command
                .arg("-preset")
                .arg(if quality_crf.is_some() { "medium" } else { "fast" })
                .arg("-crf")
                .arg(&crf_value);
        }
        "libvpx-vp9" => {
            let vp9_crf = quality_crf.unwrap_or(32).clamp(20, 45).to_string();
            command
                .arg("-row-mt")
                .arg("1")
                .arg("-speed")
    ...
    if quality_crf.is_some() {
        // Keep audio smaller for dedicated compression jobs.
...
fn video_compress_crf(quality: &str) -> Result<u32, String> {
    match quality.trim().to_ascii_lowercase().as_str() {
        "high" => Ok(20),
        "medium" => Ok(28),
        "low" => Ok(36),
        _ => Err("Unsupported video compression quality".to_string()),
    }
}

#[tauri::command]
async fn compress_video_batch(
    app_handle: tauri::AppHandle,
    input_paths: Vec<String>,
    output_dir: String,
    quality: String,
    ...
    let crf = video_compress_crf(&quality)?;
    let _conversion_guard = begin_conversion()?;
    let input_paths = validate_video_convert_inputs(&input_paths)?;
    let output_dir = validate_video_convert_output_dir(&output_dir)?;
    let (nvenc_encoder, cpu_encoder, audio_encoder, extension) = video_convert_profile("MP4")?;
    let ffmpeg_path = get_ffmpeg_path()?;
    let video_encoder = match nvenc_encoder {
        Some(encoder) if has_video_nvenc_encoder(&ffmpeg_path, encoder).await => encoder,
        _ => cpu_encoder,
